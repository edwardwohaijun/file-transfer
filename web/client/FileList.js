...
class FileList extends Component {
  ...
  upload = () => { // todo: once "upload" clicked, disable it(and <input file>) and all idle files can't be removed, until PUT has response
    ...
    publicFiles.forEach(f => {
      if (f.get("size") > this.props.fileConfig.maxFileSize){
        isFileTooBig = true;
        this.showServerMsg("文件\"" + f.get("name") + "\"大小超过" + getReadableFileSizeString(this.props.fileConfig.maxFileSize), 8000); // todo: limit filename length
        return false // List.forEach of immutableJS would return immediately if any cb returns false. This behaves differently from Array.forEach
      }
    });
    ...
    axios.put(location.pathname, {fileList, senderId: this.props.profile.get("id")})
        .then(res => {
          ...
          this.props.uploader.addTask(this.candidateFiles); // potential bug: res.data 中不包括 file obj, 而是在addTask中, 通过事先获取的<input file> 的id获取到实际file obj
  ...
  download = evt => {
    ...
    link.setAttribute('href', location.pathname + "/" + fileId);
    link.setAttribute("download", fileName.textContent);
    document.body.appendChild(link);
    link.click();
    link.remove();
